...
def _pdb_from_atoms(R, species, lattice):
    """Return a PDB string for atomic positions and lattice."""
    a, b, c = lattice
    ...
    lines = [
        f"CRYST1{la:9.3f}{lb:9.3f}{lc:9.3f}{alpha:7.2f}{beta:7.2f}{gamma:7.2f} P 1           1",
    ]
    for i, (sym, pos) in enumerate(zip(species, R), start=1):
        lines.append(
            f"HETATM{i:5d} {sym:>2}   MOL A   1    {pos[0]:8.3f}{pos[1]:8.3f}{pos[2]:8.3f}  1.00  0.00          {sym:>2}"
        )
    lines.append("END")
    return "\n".join(lines)


def make_mode_animation_widget(R, species, lattice, eigenvectors, qvec, amp=0.2, steps=24):
    """Create an NGLView widget animating a phonon mode."""
    N = len(R)
    nat = len(eigenvectors)
    ev = np.array(eigenvectors, dtype=complex)
    ...
    traj = nv.TextTrajectory(pdbs, ext="pdb")
